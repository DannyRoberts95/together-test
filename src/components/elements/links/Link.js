...
export function Link(props) {
	...
	const defaultLinkClasses = `inline-block ${className || ""} ${
		url.length === 0 || url === "#" ? "pointer-events-none cursor-default" : ""
	}`;
	...
	return isExternal ? (
		<a
			...
			className={`${defaultLinkClasses} flex-wrap-none flex items-center`}
			{...other}
		>
			<LinkTypeRenderer {...props} />
			{link?.startIcon && <div className="mr-2">{link.startIcon}</div>}
			{!hideExternalArrow && <ArrowExternal className="ml-2" />}
		</a>
	) : (
...
export function LinkTypeRenderer(props) {
	...
	return (
		<>
			{!type && !children && link?.title?.length > 0 && (
